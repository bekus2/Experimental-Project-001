...
$stmt = $pdo->prepare('SELECT id, username, avatar_color, bio, role, created_at FROM users WHERE username = ?');
...
$topics = $pdo->query("
    SELECT t.id, t.title, t.last_post_at, c.title AS cat_title, c.slug AS cat_slug,
        ...
        <?php foreach ($topics as $t): ?>
            <div class="topic-row">
                <div class="topic-main">
                    <p class="topic-title"><a href="<?= url('topic.php?id=' . (int)$t['id']) ?>"><?= e($t['title']) ?></a></p>
                    <p class="topic-sub">
